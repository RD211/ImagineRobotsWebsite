...
class SpaceBehaviour extends Behaviour {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (Star star in stars) {
      ...
      var x = star.targetPosition.dx / star.distance * 1.05;
      var y = star.targetPosition.dy / star.distance * 1.05;
      double z = 1.0 / star.distance * 6.0 + 5.0;
      paint.strokeWidth = z;
      canvas.drawLine(
        Offset(x, y),
        star.position,
  ...
  bool tick(double delta, Duration elapsed) {
    delta-=0.01;
    center = Offset.lerp(center, targetCenter, delta * 50.0);
    for (Star star in stars) {
      star.position = Offset(
        star.targetPosition.dx / star.distance,
        star.targetPosition.dy / star.distance,
