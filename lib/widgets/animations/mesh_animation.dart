  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (Particle particle in particles) {
      ...
      if (_particleImage != null) {
        ...
        {
          if(math.max(p.cx, particle.cx) - math.min(p.cx, particle.cx)<100&&math.max(p.cy, particle.cy) - math.min(p.cy, particle.cy)<100)
          {
            canvas.drawLine(Offset(p.cx,p.cy), Offset(particle.cx,particle.cy), _paint);
            counter++;
          }
          if(counter==5) break;
        }
